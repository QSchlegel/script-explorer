...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for utxo_view columns
    const filters = {
    ...
    // Execute the query
    const utxoRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return utxoRecords;
  } catch (err) {
    console.error('Error in /api/utxo_view:', err.message);
