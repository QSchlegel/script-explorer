...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for epoch_stake columns
    const filters = {
    ...
    const queryString = `
      SELECT id, addr_id, pool_id, amount, epoch_no
      FROM epoch_stake
      ${whereClause}
      ORDER BY id ${orderBy}
    ...
    // Execute the query
    const epochStakeRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return epochStakeRecords;
  } catch (err) {
    console.error('Error in /api/epoch_stake:', err.message);
