<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Datum Data</h1>
      ...
      <!-- Datum Table -->
      <TableView v-if="!loading" :headers="headers" :rows="datumRecords" :columns="columns" />
  ...
  const filterFields = reactive([
    ...
    { name: 'hash', type: 'text', placeholder: 'Enter Hash', showMinMax: false },
    { name: 'tx_id', type: 'text', placeholder: 'Enter Tx ID', showMinMax: true },
    { name: 'value', type: 'text', placeholder: 'Enter JSON Value', showMinMax: false },
  ]);
  
  // Fetch data from /api/datum
  const fetchDatumRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/datum?${queryParams}`);
      datumRecords.value = data;
