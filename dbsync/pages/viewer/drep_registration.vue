<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">DRep Registration Data</h1>
      ...
      <!-- DRep Registration Table -->
      <TableView v-if="!loading" :headers="headers" :rows="drepRegistrations" :columns="columns" />
  ...
  const headers = ['ID', 'Tx ID', 'Cert Index', 'Deposit', 'DRep Hash ID', 'Voting Anchor ID'];
  const columns = ['id', 'tx_id', 'cert_index', 'deposit', 'drep_hash_id', 'voting_anchor_id'];
  
  // Reactive state for API data
  const drepRegistrations = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'cert_index', type: 'text', placeholder: 'Enter Cert Index', showMinMax: true },
    { name: 'deposit', type: 'text', placeholder: 'Enter Deposit', showMinMax: true },
    { name: 'drep_hash_id', type: 'text', placeholder: 'Enter DRep Hash ID', showMinMax: false },
    { name: 'voting_anchor_id', type: 'text', placeholder: 'Enter Voting Anchor ID', showMinMax: false },
  ]);
  
  // Fetch data from /api/drep_registration
  const fetchDrepRegistrations = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/drep_registration?${queryParams}`);
      drepRegistrations.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchDrepRegistrations(filters);
  };
