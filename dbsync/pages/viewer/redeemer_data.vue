<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Redeemer Data Records</h1>
      ...
      <!-- Redeemer Data Table -->
      <TableView v-if="!loading" :headers="headers" :rows="redeemerDataRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Hash', 'TX ID', 'Value', 'Bytes'];
  const columns = ['id', 'hash', 'tx_id', 'value', 'bytes'];
  
  // Reactive state for API data
  const redeemerDataRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'value', type: 'text', placeholder: 'Enter Value' },
    { name: 'bytes', type: 'text', placeholder: 'Enter Bytes' }
  ]);
  
  // Fetch data from /api/redeemer_data
  const fetchRedeemerDataRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/redeemer_data?${queryParams}`);
      redeemerDataRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchRedeemerDataRecords(filters);
  };
