<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Committee Data</h1>
      ...
      <!-- Committee Table -->
      <TableView v-if="!loading" :headers="headers" :rows="committeeRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Gov Action Proposal ID', 'Quorum Numerator', 'Quorum Denominator'];
  const columns = ['id', 'gov_action_proposal_id', 'quorum_numerator', 'quorum_denominator'];
  
  // Reactive state for API data
  const committeeRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'quorum_numerator', type: 'text', placeholder: 'Enter Quorum Numerator', showMinMax: true },
    { name: 'quorum_denominator', type: 'text', placeholder: 'Enter Quorum Denominator', showMinMax: true },
  ]);
  
  // Fetch data from /api/committee
  const fetchCommitteeRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/committee?${queryParams}`);
      committeeRecords.value = data;
