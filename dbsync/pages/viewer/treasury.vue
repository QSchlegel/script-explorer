<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Treasury Records</h1>
      ...
      <!-- Treasury Table -->
      <TableView v-if="!loading" :headers="headers" :rows="treasuryRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Address ID', 'Cert Index', 'Amount', 'Transaction ID'];
  const columns = ['id', 'addr_id', 'cert_index', 'amount', 'tx_id'];
  
  // Reactive state for API data
  const treasuryRecords = ref([]);
  ...
  // Fetch data from /api/treasury
  const fetchTreasuryRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/treasury?${queryParams}`);
      treasuryRecords.value = data;
