<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Pool Retire Data</h1>
      ...
      <!-- Pool Retire Table -->
      <TableView v-if="!loading" :headers="headers" :rows="poolRetireRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Hash ID', 'Cert Index', 'Announced TX ID', 'Retiring Epoch'];
  const columns = ['id', 'hash_id', 'cert_index', 'announced_tx_id', 'retiring_epoch'];
  
  // Reactive state for API data
  const poolRetireRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'announced_tx_id', type: 'text', placeholder: 'Enter Announced TX ID' },
    { name: 'retiring_epoch', type: 'text', placeholder: 'Enter Retiring Epoch', showMinMax: true }
  ]);
  
  // Fetch data from /api/pool_retire
  const fetchPoolRetireRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/pool_retire?${queryParams}`);
      poolRetireRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchPoolRetireRecords(filters);
  };
