<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Pool Metadata Reference Data</h1>
      ...
      <!-- Pool Metadata Reference Table -->
      <TableView v-if="!loading" :headers="headers" :rows="poolMetadataRefRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Pool ID', 'URL', 'Hash', 'Registered TX ID'];
  const columns = ['id', 'pool_id', 'url', 'hash', 'registered_tx_id'];
  
  // Reactive state for API data
  const poolMetadataRefRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'url', type: 'text', placeholder: 'Enter URL' },
    { name: 'hash', type: 'text', placeholder: 'Enter Hash' },
    { name: 'registered_tx_id', type: 'text', placeholder: 'Enter Registered TX ID' }
  ]);
  
  // Fetch data from /api/pool_metadata_ref
  const fetchPoolMetadataRefRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/pool_metadata_ref?${queryParams}`);
      poolMetadataRefRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchPoolMetadataRefRecords(filters);
  };
