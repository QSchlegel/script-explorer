...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for reward columns
    const filters = {
    ...
    const queryString = `
      SELECT addr_id, type, amount, spendable_epoch, pool_id, earned_epoch
      FROM reward
      ${whereClause}
    ...
    // Execute the query
    const rewardRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return rewardRecords;
  } catch (err) {
    console.error('Error in /api/reward:', err.message);
