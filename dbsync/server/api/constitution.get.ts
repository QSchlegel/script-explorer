...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for constitution columns
    const filters = {
    ...
    const queryString = `
      SELECT id, gov_action_proposal_id, voting_anchor_id, encode(script_hash, 'hex') AS script_hash
      FROM constitution
      ${whereClause}
      ORDER BY id ${orderBy}
    ...
    // Execute the query
    const constitutionRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return constitutionRecords;
  } catch (err) {
    console.error('Error in /api/constitution:', err.message);
