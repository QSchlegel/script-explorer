<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Stake Address Records</h1>
      ...
      <!-- Stake Address Table -->
      <TableView v-if="!loading" :headers="headers" :rows="stakeAddressRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Hash Raw', 'View', 'Script Hash'];
  const columns = ['id', 'hash_raw', 'view', 'script_hash'];
  
  // Reactive state for API data
  const stakeAddressRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'hash_raw', type: 'text', placeholder: 'Enter Hash Raw' },
    { name: 'view', type: 'text', placeholder: 'Enter View' },
    { name: 'script_hash', type: 'text', placeholder: 'Enter Script Hash' }
  ]);
  
  // Fetch data from /api/stake_address
  const fetchStakeAddressRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/stake_address?${queryParams}`);
      stakeAddressRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchStakeAddressRecords(filters);
  };
