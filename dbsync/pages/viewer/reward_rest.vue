<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Reward Rest Records</h1>
      ...
      <!-- Reward Rest Table -->
      <TableView v-if="!loading" :headers="headers" :rows="rewardRestRecords" :columns="columns" />
  ...
  const headers = ['Address ID', 'Type', 'Amount', 'Spendable Epoch', 'Earned Epoch'];
  const columns = ['addr_id', 'type', 'amount', 'spendable_epoch', 'earned_epoch'];
  
  // Reactive state for API data
  const rewardRestRecords = ref([]);
  ...
  const filterFields = reactive([
    { name: 'addr_id', type: 'text', placeholder: 'Enter Address ID' },
    { name: 'type', type: 'text', placeholder: 'Enter Type' },
    { name: 'amount', type: 'text', placeholder: 'Enter Amount', showMinMax: true },
    { name: 'spendable_epoch', type: 'text', placeholder: 'Enter Spendable Epoch', showMinMax: true },
    { name: 'earned_epoch', type: 'text', placeholder: 'Enter Earned Epoch', showMinMax: true }
  ]);
  
  // Fetch data from /api/reward_rest
  const fetchRewardRestRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/reward_rest?${queryParams}`);
      rewardRestRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchRewardRestRecords(filters);
  };
