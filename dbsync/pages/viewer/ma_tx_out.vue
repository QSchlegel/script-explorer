<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">MA Tx Out Data</h1>
      ...
      <!-- MA Tx Out Table -->
      <TableView v-if="!loading" :headers="headers" :rows="maTxOuts" :columns="columns" />
  ...
  const headers = ['ID', 'Quantity', 'Tx Out ID', 'Ident'];
  const columns = ['id', 'quantity', 'tx_out_id', 'ident'];
  ...
  const filterFields = reactive([
    ...
    { name: 'quantity', type: 'text', placeholder: 'Enter Quantity', showMinMax: true },
    { name: 'tx_out_id', type: 'text', placeholder: 'Enter Tx Out ID', showMinMax: true },
    { name: 'ident', type: 'text', placeholder: 'Enter Ident', showMinMax: false },
  ]);
  
  // Fetch data from /api/ma_tx_out
  const fetchMaTxOuts = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/ma_tx_out?${queryParams}`);
      maTxOuts.value = data;
