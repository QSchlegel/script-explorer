<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Pot Transfer Data</h1>
      ...
      <!-- Pot Transfer Table -->
      <TableView v-if="!loading" :headers="headers" :rows="potTransferRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Cert Index', 'Treasury', 'Reserves', 'TX ID'];
  const columns = ['id', 'cert_index', 'treasury', 'reserves', 'tx_id'];
  
  // Reactive state for API data
  const potTransferRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'treasury', type: 'text', placeholder: 'Enter Treasury Amount', showMinMax: true },
    { name: 'reserves', type: 'text', placeholder: 'Enter Reserves Amount', showMinMax: true },
    { name: 'tx_id', type: 'text', placeholder: 'Enter TX ID' }
  ]);
  
  // Fetch data from /api/pot_transfer
  const fetchPotTransferRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/pot_transfer?${queryParams}`);
      potTransferRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchPotTransferRecords(filters);
  };
