<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Epoch State Data</h1>
      ...
      <!-- Epoch State Table -->
      <TableView v-if="!loading" :headers="headers" :rows="epochStateRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Committee ID', 'No Confidence ID', 'Constitution ID', 'Epoch No'];
  const columns = ['id', 'committee_id', 'no_confidence_id', 'constitution_id', 'epoch_no'];
  
  // Reactive state for API data
  const epochStateRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'committee_id', type: 'text', placeholder: 'Enter Committee ID' },
    { name: 'no_confidence_id', type: 'text', placeholder: 'Enter No Confidence ID' },
    { name: 'constitution_id', type: 'text', placeholder: 'Enter Constitution ID' },
    { name: 'epoch_no', type: 'text', placeholder: 'Enter Epoch No', showMinMax: true }
  ]);
  
  // Fetch data from /api/epoch_state
  const fetchEpochStateRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/epoch_state?${queryParams}`);
      epochStateRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchEpochStateRecords(filters);
  };
