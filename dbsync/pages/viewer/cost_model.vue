<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Cost Model Data</h1>
      ...
      <!-- Cost Model Table -->
      <TableView v-if="!loading" :headers="headers" :rows="costModelRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Costs', 'Hash'];
  const columns = ['id', 'costs', 'hash'];
  
  // Reactive state for API data
  const costModelRecords = ref([]);
  ...
  // Fetch data from /api/cost_model
  const fetchCostModelRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/cost_model?${queryParams}`);
      costModelRecords.value = data;
