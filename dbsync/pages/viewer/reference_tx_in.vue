<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Reference TX In Records</h1>
      ...
      <!-- Reference TX In Table -->
      <TableView v-if="!loading" :headers="headers" :rows="referenceTxInRecords" :columns="columns" />
  ...
  const headers = ['ID', 'TX In ID', 'TX Out ID', 'TX Out Index'];
  const columns = ['id', 'tx_in_id', 'tx_out_id', 'tx_out_index'];
  
  // Reactive state for API data
  const referenceTxInRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'tx_in_id', type: 'text', placeholder: 'Enter TX In ID' },
    { name: 'tx_out_id', type: 'text', placeholder: 'Enter TX Out ID' },
    { name: 'tx_out_index', type: 'text', placeholder: 'Enter TX Out Index' }
  ]);
  
  // Fetch data from /api/reference_tx_in
  const fetchReferenceTxInRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/reference_tx_in?${queryParams}`);
      referenceTxInRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchReferenceTxInRecords(filters);
  };
