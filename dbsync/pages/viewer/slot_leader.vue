<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Slot Leader Records</h1>
      ...
      <!-- Slot Leader Table -->
      <TableView v-if="!loading" :headers="headers" :rows="slotLeaderRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Hash', 'Pool Hash ID', 'Description'];
  const columns = ['id', 'hash', 'pool_hash_id', 'description'];
  
  // Reactive state for API data
  const slotLeaderRecords = ref([]);
  ...
  // Fetch data from /api/slot_leader
  const fetchSlotLeaderRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/slot_leader?${queryParams}`);
      slotLeaderRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchSlotLeaderRecords(filters);
  };
