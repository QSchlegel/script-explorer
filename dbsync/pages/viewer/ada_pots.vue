<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">ADA Pots Data</h1>
      ...
      <!-- ADA Pots Table -->
      <TableView v-if="!loading" :headers="headers" :rows="adaPotsRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Slot No', 'Epoch No', 'Treasury', 'Reserves', 'Rewards', 'UTXO', 'Deposits Stake', 'Fees', 'Block ID', 'Deposits DRep', 'Deposits Proposal'];
  const columns = ['id', 'slot_no', 'epoch_no', 'treasury', 'reserves', 'rewards', 'utxo', 'deposits_stake', 'fees', 'block_id', 'deposits_drep', 'deposits_proposal'];
  
  // Reactive state for API data
  const adaPotsRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'treasury', type: 'text', placeholder: 'Enter Treasury', showMinMax: true },
    { name: 'reserves', type: 'text', placeholder: 'Enter Reserves', showMinMax: true },
    { name: 'rewards', type: 'text', placeholder: 'Enter Rewards', showMinMax: true },
    { name: 'utxo', type: 'text', placeholder: 'Enter UTXO', showMinMax: true },
    { name: 'deposits_stake', type: 'text', placeholder: 'Enter Deposits Stake', showMinMax: true },
    { name: 'fees', type: 'text', placeholder: 'Enter Fees', showMinMax: true },
    { name: 'block_id', type: 'text', placeholder: 'Enter Block ID', showMinMax: false },
    { name: 'deposits_drep', type: 'text', placeholder: 'Enter Deposits DRep', showMinMax: true },
    { name: 'deposits_proposal', type: 'text', placeholder: 'Enter Deposits Proposal', showMinMax: true },
  ]);
  
  // Fetch data from /api/ada_pots
  const fetchAdaPotsRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/ada_pots?${queryParams}`);
      adaPotsRecords.value = data;
