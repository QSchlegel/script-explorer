...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for extra_migrations columns
    const filters = {
    ...
    // Execute the query
    const extraMigrationsRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return extraMigrationsRecords;
  } catch (err) {
    console.error('Error in /api/extra_migrations:', err.message);
