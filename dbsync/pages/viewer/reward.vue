<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Reward Records</h1>
      ...
      <!-- Reward Table -->
      <TableView v-if="!loading" :headers="headers" :rows="rewardRecords" :columns="columns" />
  ...
  const headers = ['Address ID', 'Type', 'Amount', 'Spendable Epoch', 'Pool ID', 'Earned Epoch'];
  const columns = ['addr_id', 'type', 'amount', 'spendable_epoch', 'pool_id', 'earned_epoch'];
  ...
  // Fetch data from /api/reward
  const fetchRewardRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/reward?${queryParams}`);
