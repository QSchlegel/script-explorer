<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Epoch Stake Progress Data</h1>
      ...
      <!-- Epoch Stake Progress Table -->
      <TableView v-if="!loading" :headers="headers" :rows="epochStakeProgressRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Epoch No', 'Completed'];
  const columns = ['id', 'epoch_no', 'completed'];
  
  // Reactive state for API data
  const epochStakeProgressRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'completed', type: 'select', options: ['true', 'false'], placeholder: 'Select Completion Status' }
  ]);
  
  // Fetch data from /api/epoch_stake_progress
  const fetchEpochStakeProgressRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/epoch_stake_progress?${queryParams}`);
      epochStakeProgressRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchEpochStakeProgressRecords(filters);
  };
  ...
  const prevPage = () => {
    if (page.value > 1) {
      page.value--;
      fetchEpochStakeProgressRecords();
    }
  };
