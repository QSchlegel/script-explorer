...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for delisted_pool columns
    const filters = {
    ...
    const queryString = `
      SELECT id, encode(hash_raw, 'hex') AS hash_raw
      FROM delisted_pool
      ${whereClause}
      ORDER BY id ${orderBy}
    ...
    // Execute the query
    const delistedPoolRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return delistedPoolRecords;
  } catch (err) {
    console.error('Error in /api/delisted_pool:', err.message);
