<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Withdrawal Records</h1>
      ...
      <!-- Withdrawal Table -->
      <TableView v-if="!loading" :headers="headers" :rows="withdrawalRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Address ID', 'Amount', 'Redeemer ID', 'Transaction ID'];
  const columns = ['id', 'addr_id', 'amount', 'redeemer_id', 'tx_id'];
  ...
  // Fetch data from /api/withdrawal
  const fetchWithdrawalRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/withdrawal?${queryParams}`);
  ...
  const applyFilter = (filters) => {
    ...
    fetchWithdrawalRecords(filters);
  };
