<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Treasury Withdrawal Records</h1>
      ...
      <!-- Treasury Withdrawal Table -->
      <TableView v-if="!loading" :headers="headers" :rows="treasuryWithdrawalRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Gov Action Proposal ID', 'Stake Address ID', 'Amount'];
  const columns = ['id', 'gov_action_proposal_id', 'stake_address_id', 'amount'];
  
  // Reactive state for API data
  const treasuryWithdrawalRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'gov_action_proposal_id', type: 'text', placeholder: 'Enter Gov Action Proposal ID' },
  ...
  // Fetch data from /api/treasury_withdrawal
  const fetchTreasuryWithdrawalRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/treasury_withdrawal?${queryParams}`);
      treasuryWithdrawalRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchTreasuryWithdrawalRecords(filters);
  };
  ...
  const prevPage = () => {
    if (page.value > 1) {
      page.value--;
      fetchTreasuryWithdrawalRecords();
    }
  };
