<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Delegation Vote Data</h1>
      ...
      <!-- Delegation Vote Table -->
      <TableView v-if="!loading" :headers="headers" :rows="delegationVoteRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Addr ID', 'Cert Index', 'DRep Hash ID', 'Tx ID', 'Redeemer ID'];
  const columns = ['id', 'addr_id', 'cert_index', 'drep_hash_id', 'tx_id', 'redeemer_id'];
  
  // Reactive state for API data
  const delegationVoteRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'drep_hash_id', type: 'text', placeholder: 'Enter DRep Hash ID' },
    { name: 'tx_id', type: 'text', placeholder: 'Enter Tx ID' },
    { name: 'redeemer_id', type: 'text', placeholder: 'Enter Redeemer ID' },
  ]);
  
  // Fetch data from /api/delegation_vote
  const fetchDelegationVoteRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/delegation_vote?${queryParams}`);
      delegationVoteRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchDelegationVoteRecords(filters);
  };
