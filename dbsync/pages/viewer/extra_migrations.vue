<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Extra Migrations Data</h1>
      ...
      <!-- Extra Migrations Table -->
      <TableView v-if="!loading" :headers="headers" :rows="extraMigrationsRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Token', 'Description'];
  const columns = ['id', 'token', 'description'];
  
  // Reactive state for API data
  const extraMigrationsRecords = ref([]);
  ...
  // Fetch data from /api/extra_migrations
  const fetchExtraMigrationsRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/extra_migrations?${queryParams}`);
      extraMigrationsRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchExtraMigrationsRecords(filters);
  };
