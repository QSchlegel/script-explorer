<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Epoch Stake Data</h1>
      ...
      <!-- Epoch Stake Table -->
      <TableView v-if="!loading" :headers="headers" :rows="epochStakeRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Address ID', 'Pool ID', 'Amount', 'Epoch No'];
  const columns = ['id', 'addr_id', 'pool_id', 'amount', 'epoch_no'];
  
  // Reactive state for API data
  const epochStakeRecords = ref([]);
  ...
  // Fetch data from /api/epoch_stake
  const fetchEpochStakeRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/epoch_stake?${queryParams}`);
      epochStakeRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchEpochStakeRecords(filters);
  };
