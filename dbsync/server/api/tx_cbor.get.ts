...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for tx_cbor columns
    const filters = {
    ...
    // Execute the query
    const txCbor = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results, converting buffer fields to hex string
    return txCbor.map(cbor => ({
      ...cbor,
      bytes: cbor.bytes ? Buffer.from(cbor.bytes).toString('hex') : null
    }));
  } catch (err) {
    console.error('Error in /api/tx_cbor:', err);
