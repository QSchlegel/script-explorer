<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Multi Asset Data</h1>
      ...
      <!-- Multi Asset Table -->
      <TableView v-if="!loading" :headers="headers" :rows="multiAssets" :columns="columns" />
  ...
  const headers = ['ID', 'Policy', 'Name', 'Fingerprint'];
  const columns = ['id', 'policy', 'name', 'fingerprint'];
  
  // Reactive state for API data
  const multiAssets = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'policy', type: 'text', placeholder: 'Enter Policy', showMinMax: false },
    { name: 'name', type: 'text', placeholder: 'Enter Name', showMinMax: false },
    { name: 'fingerprint', type: 'text', placeholder: 'Enter Fingerprint', showMinMax: false },
  ]);
  
  // Fetch data from /api/multi_asset
  const fetchMultiAssets = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/multi_asset?${queryParams}`);
      multiAssets.value = data;
