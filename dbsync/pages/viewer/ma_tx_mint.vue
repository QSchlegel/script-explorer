<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">MA Tx Mint Data</h1>
      ...
      <!-- MA Tx Mint Table -->
      <TableView v-if="!loading" :headers="headers" :rows="maTxMints" :columns="columns" />
  ...
  const headers = ['ID', 'Quantity', 'Tx ID', 'Ident'];
  const columns = ['id', 'quantity', 'tx_id', 'ident'];
  ...
  // Fetch data from /api/ma_tx_mint
  const fetchMaTxMints = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/ma_tx_mint?${queryParams}`);
      maTxMints.value = data;
