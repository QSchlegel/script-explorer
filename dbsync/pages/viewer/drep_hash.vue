<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">DRep Hash Data</h1>
      ...
      <!-- DRep Hash Table -->
      <TableView v-if="!loading" :headers="headers" :rows="drepHashes" :columns="columns" />
  ...
  const headers = ['ID', 'Raw', 'View', 'Has Script'];
  const columns = ['id', 'raw', 'view', 'has_script'];
  
  // Reactive state for API data
  const drepHashes = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'raw', type: 'text', placeholder: 'Enter Raw Hash', showMinMax: false },
    { name: 'view', type: 'text', placeholder: 'Enter View', showMinMax: false },
    { name: 'has_script', type: 'checkbox', placeholder: 'Has Script', showMinMax: false },
  ]);
  
  // Fetch data from /api/drep_hash
  const fetchDrepHashes = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/drep_hash?${queryParams}`);
      drepHashes.value = data;
