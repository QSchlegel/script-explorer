<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Reserve Records</h1>
      ...
      <!-- Reserve Table -->
      <TableView v-if="!loading" :headers="headers" :rows="reserveRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Address ID', 'Cert Index', 'Amount', 'TX ID'];
  ...
  // Fetch data from /api/reserve
  const fetchReserveRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/reserve?${queryParams}`);
      reserveRecords.value = data;
