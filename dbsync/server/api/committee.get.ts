...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for committee columns
    const filters = {
      id: { single: query.id, min: query.min_id, max: query.max_id },
      gov_action_proposal_id: { single: query.gov_action_proposal_id },
      quorum_numerator: { single: query.quorum_numerator, min: query.min_quorum_numerator, max: query.max_quorum_numerator },
      quorum_denominator: { single: query.quorum_denominator, min: query.min_quorum_denominator, max: query.max_quorum_denominator }
    };
    ...
    const queryString = `
      SELECT id, gov_action_proposal_id, quorum_numerator, quorum_denominator
      FROM committee
      ${whereClause}
      ORDER BY id ${orderBy}
    ...
    // Execute the query
    const committeeRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return committeeRecords;
  } catch (err) {
    console.error('Error in /api/committee:', err.message);
