...
<template>
    <q-card v-if="props.assetId !== undefined && infoObj !== undefined && addrObj !== undefined && txObj !== undefined"
        flat>
        <div class="row">
        ...
        <div class="row q-ml-md">
            <HoverIcon class="col-auto q-pt-lg" :icon-name="'sym_o_fingerprint'" :icon-size="'sm'" :headline="'Fingerprint'"
                :content="'This is an Asset Fingerprint.'" />
                ...
                <tbody>
                    <tr v-for="tx, index in txObj.txs" :key="index">
                        <td class="row text-left">
                            <HoverIcon class="col-auto q-mr-sm" v-if="infoObj.initial_mint_tx_hash === tx.tx_hash" :icon-name="'sym_o_loupe'"
                                :icon-size="'xs'" :headline="'Initial mint transaction'" :content="''" />
                            <router-link :to="'/txs/' + tx.tx_hash" class="col-auto text-indigo-9" style="text-decoration: none">
                                {{ tx.tx_hash }}
                            </router-link>
                        </td>
                ...
                <tbody>
                    <tr v-for="addr, index in addrObj.addrs" :key="index">
                        <td class="row text-left">
                            ...
                            <router-link :to="'/addresses/' + addr.address" class="col-auto text-indigo-9" style="text-decoration: none">
                                {{ addr.address }}
                            </router-link>
                        </td>
