...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for reference_tx_in columns
    const filters = {
      id: { single: query.id, min: query.min_id, max: query.max_id },
      tx_in_id: { single: query.tx_in_id },
      tx_out_id: { single: query.tx_out_id },
      tx_out_index: { single: query.tx_out_index }
    };
    ...
    // Execute the query
    const referenceTxInRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return referenceTxInRecords;
  } catch (err) {
    console.error('Error in /api/reference_tx_in:', err.message);
