<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Constitution Data</h1>
      ...
      <!-- Constitution Table -->
      <TableView v-if="!loading" :headers="headers" :rows="constitutionRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Gov Action Proposal ID', 'Voting Anchor ID', 'Script Hash'];
  const columns = ['id', 'gov_action_proposal_id', 'voting_anchor_id', 'script_hash'];
  
  // Reactive state for API data
  const constitutionRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'gov_action_proposal_id', type: 'text', placeholder: 'Enter Gov Action Proposal ID', showMinMax: false },
    { name: 'voting_anchor_id', type: 'text', placeholder: 'Enter Voting Anchor ID', showMinMax: false },
    { name: 'script_hash', type: 'text', placeholder: 'Enter Script Hash', showMinMax: false },
  ]);
  
  // Fetch data from /api/constitution
  const fetchConstitutionRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/constitution?${queryParams}`);
      constitutionRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchConstitutionRecords(filters);
  };
