<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Off-Chain Pool Data</h1>
      ...
      <!-- Off-Chain Pool Data Table -->
      <TableView v-if="!loading" :headers="headers" :rows="offChainPoolDataRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Pool ID', 'Ticker Name', 'Hash', 'JSON', 'Bytes', 'PMR ID'];
  const columns = ['id', 'pool_id', 'ticker_name', 'hash', 'json', 'bytes', 'pmr_id'];
  
  // Reactive state for API data
  const offChainPoolDataRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'ticker_name', type: 'text', placeholder: 'Enter Ticker Name' },
    { name: 'hash', type: 'text', placeholder: 'Enter Hash' },
    { name: 'pmr_id', type: 'text', placeholder: 'Enter PMR ID' }
  ]);
  
  // Fetch data from /api/off_chain_pool_data
  const fetchOffChainPoolDataRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/off_chain_pool_data?${queryParams}`);
      offChainPoolDataRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchOffChainPoolDataRecords(filters);
  };
