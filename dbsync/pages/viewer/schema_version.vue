<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Schema Version Records</h1>
      ...
      <TableView v-if="!loading" :headers="headers" :rows="schemaVersionRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Stage One', 'Stage Two', 'Stage Three'];
  const columns = ['id', 'stage_one', 'stage_two', 'stage_three'];
  
  // Reactive state for API data
  const schemaVersionRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'stage_one', type: 'text', placeholder: 'Enter Stage One' },
    { name: 'stage_two', type: 'text', placeholder: 'Enter Stage Two' },
    { name: 'stage_three', type: 'text', placeholder: 'Enter Stage Three' }
  ]);
  
  // Fetch data from /api/schema_version
  const fetchSchemaVersionRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/schema_version?${queryParams}`);
      schemaVersionRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchSchemaVersionRecords(filters);
  };
