...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Execute the query
    const reverseIndexRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return reverseIndexRecords;
  } catch (err) {
    console.error('Error in /api/reverse_index:', err.message);
