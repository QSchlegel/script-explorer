<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Off-Chain Pool Fetch Error Data</h1>
      ...
      <!-- Off-Chain Pool Fetch Error Table -->
      <TableView v-if="!loading" :headers="headers" :rows="offChainPoolFetchErrorRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Pool ID', 'Fetch Time', 'PMR ID', 'Fetch Error', 'Retry Count'];
  const columns = ['id', 'pool_id', 'fetch_time', 'pmr_id', 'fetch_error', 'retry_count'];
  
  // Reactive state for API data
  const offChainPoolFetchErrorRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'pool_id', type: 'text', placeholder: 'Enter Pool ID' },
    { name: 'fetch_time', type: 'date', placeholder: 'Select Fetch Time' },
    { name: 'pmr_id', type: 'text', placeholder: 'Enter PMR ID' },
    { name: 'fetch_error', type: 'text', placeholder: 'Enter Fetch Error' },
    { name: 'retry_count', type: 'text', placeholder: 'Enter Retry Count', showMinMax: true }
  ]);
  
  // Fetch data from /api/off_chain_pool_fetch_error
  const fetchOffChainPoolFetchErrorRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/off_chain_pool_fetch_error?${queryParams}`);
      offChainPoolFetchErrorRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchOffChainPoolFetchErrorRecords(filters);
  };
  ...
  const prevPage = () => {
    if (page.value > 1) {
      page.value--;
      fetchOffChainPoolFetchErrorRecords();
    }
  };
