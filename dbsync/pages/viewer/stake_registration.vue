<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Stake Registration Records</h1>
      ...
      <!-- Stake Registration Table -->
      <TableView v-if="!loading" :headers="headers" :rows="stakeRegistrationRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Address ID', 'Cert Index', 'Epoch No', 'Transaction ID', 'Deposit'];
  const columns = ['id', 'addr_id', 'cert_index', 'epoch_no', 'tx_id', 'deposit'];
  
  // Reactive state for API data
  const stakeRegistrationRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'deposit', type: 'text', placeholder: 'Enter Deposit Amount', showMinMax: true }
  ]);
  
  // Fetch data from /api/stake_registration
  const fetchStakeRegistrationRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/stake_registration?${queryParams}`);
      stakeRegistrationRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchStakeRegistrationRecords(filters);
  };
