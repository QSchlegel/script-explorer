<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Delisted Pool Data</h1>
      ...
      <!-- Delisted Pool Table -->
      <TableView v-if="!loading" :headers="headers" :rows="delistedPoolRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Hash Raw'];
  const columns = ['id', 'hash_raw'];
  
  // Reactive state for API data
  const delistedPoolRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'hash_raw', type: 'text', placeholder: 'Enter Hash Raw', showMinMax: false },
  ]);
  
  // Fetch data from /api/delisted_pool
  const fetchDelistedPoolRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/delisted_pool?${queryParams}`);
      delistedPoolRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchDelistedPoolRecords(filters);
  };
