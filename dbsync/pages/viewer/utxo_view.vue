<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">UTXO View Records</h1>
      ...
      <!-- UTXO View Table -->
      <TableView v-if="!loading" :headers="headers" :rows="utxoRecords" :columns="columns" />
  ...
  // Fetch data from /api/utxo_view
  const fetchUtxoRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/utxo_view?${queryParams}`);
      utxoRecords.value = data;
