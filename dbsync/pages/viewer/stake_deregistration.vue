<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Stake Deregistration Records</h1>
      ...
      <!-- Stake Deregistration Table -->
      <TableView v-if="!loading" :headers="headers" :rows="stakeDeregistrationRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Address ID', 'Cert Index', 'Epoch No', 'Transaction ID', 'Redeemer ID'];
  const columns = ['id', 'addr_id', 'cert_index', 'epoch_no', 'tx_id', 'redeemer_id'];
  
  // Reactive state for API data
  const stakeDeregistrationRecords = ref([]);
  ...
  // Fetch data from /api/stake_deregistration
  const fetchStakeDeregistrationRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/stake_deregistration?${queryParams}`);
      stakeDeregistrationRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchStakeDeregistrationRecords(filters);
  };
  ...
  const prevPage = () => {
    if (page.value > 1) {
      page.value--;
      fetchStakeDeregistrationRecords();
    }
  };
