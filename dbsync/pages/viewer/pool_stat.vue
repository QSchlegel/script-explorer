<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Pool Stat Data</h1>
      ...
      <!-- Pool Stat Table -->
      <TableView v-if="!loading" :headers="headers" :rows="poolStatRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Pool Hash ID', 'Epoch No', 'Number of Blocks', 'Number of Delegators', 'Stake', 'Voting Power'];
  const columns = ['id', 'pool_hash_id', 'epoch_no', 'number_of_blocks', 'number_of_delegators', 'stake', 'voting_power'];
  
  // Reactive state for API data
  const poolStatRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'number_of_blocks', type: 'text', placeholder: 'Enter Number of Blocks', showMinMax: true },
    { name: 'number_of_delegators', type: 'text', placeholder: 'Enter Number of Delegators', showMinMax: true },
    { name: 'stake', type: 'text', placeholder: 'Enter Stake', showMinMax: true },
    { name: 'voting_power', type: 'text', placeholder: 'Enter Voting Power', showMinMax: true }
  ]);
  
  // Fetch data from /api/pool_stat
  const fetchPoolStatRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/pool_stat?${queryParams}`);
      poolStatRecords.value = data;
