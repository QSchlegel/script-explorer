<template> 
    <q-list>
        <q-item   v-for="s, index in store.ScriptList" :key="index">
            <q-item-section>
                <q-btn :style='{"background-color": (store.CurScript == s)? "lightgrey": "white"}' @click="store.CurScript = s; store.loadScriptData()">
        ...
        <q-item>
            <q-item-section>
                <q-btn class="q-mb-xl" color="accent" text-color="white" @click="store.loadMoreScripts">
                    Load Scripts
                </q-btn>
            </q-item-section>
        </q-item>
