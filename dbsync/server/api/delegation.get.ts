...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for delegation columns
    const filters = {
      ...
      active_epoch_no: { single: query.active_epoch_no },
      tx_id: { single: query.tx_id },
      slot_no: { single: query.slot_no },
    ...
    const queryString = `
      SELECT id, addr_id, cert_index, pool_hash_id, active_epoch_no, tx_id, slot_no, redeemer_id
      FROM delegation
      ${whereClause}
      ORDER BY id ${orderBy}
    ...
    // Execute the query
    const delegationRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return delegationRecords;
  } catch (err) {
    console.error('Error in /api/delegation:', err.message);
