<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Epoch Data</h1>
      ...
      <!-- Epoch Table -->
      <TableView v-if="!loading" :headers="headers" :rows="epochRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Out Sum', 'Fees', 'Tx Count', 'Blk Count', 'Epoch No', 'Start Time', 'End Time'];
  const columns = ['id', 'out_sum', 'fees', 'tx_count', 'blk_count', 'no', 'start_time', 'end_time'];
  
  // Reactive state for API data
  const epochRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'no', type: 'text', placeholder: 'Enter Epoch No', showMinMax: true },
    ...
    { name: 'blk_count', type: 'text', placeholder: 'Enter Blk Count', showMinMax: true },
    { name: 'start_time', type: 'date', placeholder: 'Enter Start Time' },
    { name: 'end_time', type: 'date', placeholder: 'Enter End Time' },
  ]);
  
  // Fetch data from /api/epoch
  const fetchEpochRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/epoch?${queryParams}`);
      epochRecords.value = data;
