...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for stake_registration columns
    const filters = {
      ...
      deposit: { min: query.min_deposit, max: query.max_deposit }
    };
    ...
    const queryString = `
      SELECT id, addr_id, cert_index, epoch_no, tx_id, deposit
    ...
    // Execute the query
    const stakeRegistrationRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return stakeRegistrationRecords;
  } catch (err) {
    console.error('Error in /api/stake_registration:', err.message);
