<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Reserved Pool Ticker Records</h1>
      ...
      <!-- Reserved Pool Ticker Table -->
      <TableView v-if="!loading" :headers="headers" :rows="reservedPoolTickerRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Name', 'Pool Hash'];
  const columns = ['id', 'name', 'pool_hash'];
  
  // Reactive state for API data
  const reservedPoolTickerRecords = ref([]);
  ...
  const filterFields = reactive([
    ...
    { name: 'pool_hash', type: 'text', placeholder: 'Enter Pool Hash' }
  ]);
  
  // Fetch data from /api/reserved_pool_ticker
  const fetchReservedPoolTickerRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/reserved_pool_ticker?${queryParams}`);
      reservedPoolTickerRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchReservedPoolTickerRecords(filters);
  };
  ...
  const prevPage = () => {
    if (page.value > 1) {
      page.value--;
      fetchReservedPoolTickerRecords();
    }
  };
