<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Pool Hash Data</h1>
      ...
      <!-- Pool Hash Table -->
      <TableView v-if="!loading" :headers="headers" :rows="poolHashRecords" :columns="columns" />
  ...
  const headers = ['ID', 'Hash Raw', 'View'];
  const columns = ['id', 'hash_raw', 'view'];
  
  // Reactive state for API data
  const poolHashRecords = ref([]);
  ...
  // Fetch data from /api/pool_hash
  const fetchPoolHashRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/pool_hash?${queryParams}`);
      poolHashRecords.value = data;
