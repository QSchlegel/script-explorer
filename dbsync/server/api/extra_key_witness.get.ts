...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for extra_key_witness columns
    const filters = {
    ...
    const queryString = `
      SELECT id, encode(hash, 'hex') AS hash, tx_id
    ...
    // Execute the query
    const extraKeyWitnessRecords = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return extraKeyWitnessRecords;
  } catch (err) {
    console.error('Error in /api/extra_key_witness:', err.message);
