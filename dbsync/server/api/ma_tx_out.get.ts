...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Define filters for ma_tx_out columns
    const filters = {
    ...
    const queryString = `
      SELECT id, quantity, tx_out_id, ident
      FROM ma_tx_out
      ${whereClause}
      ORDER BY id ${orderBy}
    ...
    // Execute the query
    const maTxOuts = await sql.unsafe(queryString, [...values, limit, offset]);

    // Return the results
    return maTxOuts;
  } catch (err) {
    console.error('Error in /api/ma_tx_out:', err.message);
