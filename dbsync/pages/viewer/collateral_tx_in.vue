<template>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-semibold mb-6 text-gray-800">Collateral Tx In Data</h1>
      ...
      <!-- Collateral Tx In Table -->
      <TableView v-if="!loading" :headers="headers" :rows="collateralTxInRecords" :columns="columns" />
  ...
  const collateralTxInRecords = ref([]);
  ...
  // Fetch data from /api/collateral_tx_in
  const fetchCollateralTxInRecords = async (filters = {}) => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      const data = await $fetch(`/api/collateral_tx_in?${queryParams}`);
      collateralTxInRecords.value = data;
  ...
  const applyFilter = (filters) => {
    ...
    fetchCollateralTxInRecords(filters);
  };
